...
<div class="container">
	<div class="vertical-offset-50">
		...
		<?php
			...
			if ($row = mysqli_fetch_row($result)) {
				...
				if ($row2 = mysqli_fetch_row($result2)) {
					echo '<div class="dropzone" id="drop-1" ondragover="allowDrop(event)" ondrop="drop(event)"><div class="wrapper"><a href="lib/del.php?id='.$row2[0].'&uid='.$_GET['id'].'"><span class="glyphicon glyphicon-remove-sign"></span></a></div><img id="'.$row2[0].'" src="admin/uploads/'.$row2[3].'" width="200px" height="200px" dragable="true" ondragstart="drag(event)"></img></div>';
				}
			} else {
				echo '<div class="dropzone" id="drop-1" ondragover="allowDrop(event)" ondrop="drop(event)"></div>';
			}
		?>
		</div>
		<div class="col-sm-8">
		...
		<?php
			...
			if (mysqli_num_rows($result) > 0) {
				...
				$result2 = mysqli_query($con, $sql2);
				if ($row2 = mysqli_fetch_row($result2)) {
					echo '<div class="col-sm-4 dropzone" id="'.$row['DivID'].'" ondragover="allowDrop(event)" ondrop="drop(event)"></img><div class="wrapper"><a href="lib/del.php?id='.$row2[0].'&uid='.$_GET['id'].'"><span class="glyphicon glyphicon-remove-sign"></span></a></div><img id="'.$row2[0].'" src="admin/uploads/'.$row2[3].'" width="200px" height="200px" dragable="true" ondragstart="drag(event)"></div>';
				}
				}
			} else {
				echo '<div class="col-sm-4 dropzone" id="drop-2" ondragover="allowDrop(event)" ondrop="drop(event)"></div>';
			}
		?>
		</div>
		</div>
	...
	<div>
		<?php
			...
			while($row = mysqli_fetch_assoc($result)) {
			echo '
			<img id="'.$row['ID'].'" class="img" src="admin/uploads/'.$row['Image'].'" draggable="true" ondragstart="drag(event)" width="200px" height="200px"></img>
			';
			}
			} else {
			echo "0 results";
...
function add() {
	document.getElementById('then').innerHTML += '<div class="col-sm-4 dropzone" id="drop-' + makeid() + '" ondragover="allowDrop(event)" ondrop="drop(event)"></div>';
	sizer("then");
}
sizer("then");
</script>
<script type="text/javascript">
  <!--
  if (screen.width <= 800) {
    window.location = "http://school.jehanwadia.ca/team10/mobile.php?id=<?php echo $uid; ?>";
  }
  //-->
</script>
</body>
